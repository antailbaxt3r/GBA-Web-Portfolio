...
export function dpadSprite() {
  const S = 60, A = 20;
  const b = new Bitmap(S, S);
  const face = [248, 248, 248, 150];
  const edge = [40, 40, 56, 190];
  const arm = (x, y, w, h) => { b.fill(x, y, w, h, face); b.rect(x, y, w, h, edge); };
  arm(A, 0, A, S);
  arm(0, A, S, A);
  b.fill(A + 1, A + 1, A - 2, A - 2, face);
  // arrow glyphs
  const tri = (cx, cy, dx, dy) => {
    for (let i = 0; i < 5; i++) for (let j = -i; j <= i; j++) {
      b.put(cx + dx * i + (dx === 0 ? j : 0), cy + dy * i + (dy === 0 ? j : 0), edge);
    }
  };
  tri(S / 2, 4, 0, 1); tri(S / 2, S - 5, 0, -1);
  tri(4, S / 2, 1, 0); tri(S - 5, S / 2, -1, 0);
  return b;
}

export function roundButton(label) {
  const S = 28;
  const b = new Bitmap(S, S);
  const c = (S - 1) / 2;
  for (let y = 0; y < S; y++) for (let x = 0; x < S; x++) {
    const r = Math.sqrt((x - c) ** 2 + (y - c) ** 2);
    if (r > 13.5) continue;
    b.put(x, y, r > 11.5 ? [40, 40, 56, 200] : [248, 248, 248, 150]);
  }
  const glyph = {
    A: ['.##.', '#..#', '####', '#..#', '#..#'],
    B: ['###.', '#..#', '###.', '#..#', '###.'],
  }[label];
  // Integer origin: `c` is 13.5, and a fractional index into the RGBA buffer
  // writes across channel boundaries and corrupts neighbouring pixels.
  const gx = Math.round(c) - 2;
  const gy = Math.round(c) - 2;
  glyph.forEach((row, y) => {
    for (let x = 0; x < row.length; x++) {
      if (row[x] === '#') b.put(gx + x, gy + y, [40, 40, 56, 220]);
    }
  });
  return b;
}
